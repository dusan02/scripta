...
export async function queryFirmy(
  ...
  // regardless of filters (PostgreSQL prefers seq scan for COUNT).
  // Approximate count is instant and accurate enough for pagination UI.
  ...
  `;
  const total = Number(approx[0]?.estimate ?? 0);
