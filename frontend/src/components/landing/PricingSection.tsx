      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pricing-grid" style={{ alignItems: "stretch" }}>
          {PRICING_PLANS.map((plan) => (
            <div key={plan.id} style={{ background: "var(--surface)", border: plan.highlight ? "2px solid var(--accent)" : "1px solid var(--border)", borderRadius: 16, padding: 26, position: "relative", boxShadow: plan.highlight ? "var(--shadow-lg)" : "var(--shadow-sm)" }}>
              {plan.highlight && (
                <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: "var(--accent)", color: "var(--accent-button-text)", padding: "4px 14px", borderRadius: 999, fontSize: 11, fontWeight: 700 }}>
                  {t("home.popular")}
                </div>
              )}
              <h3 style={{ fontSize: 17, fontWeight: 800, marginBottom: 2 }}>{t(plan.nameKey)}</h3>
              <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>{t(plan.subtitleKey)}</p>
              <div style={{ fontSize: 28, fontWeight: 900, marginBottom: 4 }}>{plan.price} €{plan.isSubscription ? " / mesiac" : ""}</div>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 32, marginTop: 36, flexWrap: "wrap", fontSize: 13, color: "var(--text-secondary)" }} className="pricing-guarantee">
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ color: "var(--accent)", fontSize: 16 }}>🛡️</span>
