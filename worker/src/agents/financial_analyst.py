...
- `hruba_marza` (Gross profit): V SK GAAP hľadaj IBA riadok 'Hrubý zisk' / 'Gross profit'. V IFRS hľadaj 'Gross profit'. NIKDY nepoužívaj 'Pridaná hodnota' (Value added) ako proxy — to je odlišná metrika. NIKDY nedopočítavaj matematicky (Revenue - COGS)! Ak riadok 'Hrubý zisk' / 'Gross profit' nie je explicitne uvedený v dokumente ako číslo, vráť null. Systém ju neskôr dopočíta sám.
...
- `uroky` (Interest expense): Hľadaj vo Výkaze ziskov a strát: 'Úroky', 'Náklady na úroky', 'Interest expense', 'Interest payable'. Extrahuj VŽDY ako kladné číslo (náklad je kladný). DÔLEŽITÉ: 'Finance costs' v IFRS je širšia kategória (zahŕňa úroky + kurzové rozdiely + bankové poplatky) — NIKDY nepoužívaj 'Finance costs' ako hodnotu pre `uroky`. Hľadaj IBA riadok explicitne označený ako 'Interest expense' / 'Interest payable' / 'Úroky' / 'Náklady na úroky'. Ak dokument uvádza len 'Finance costs' bez rozkladu na interest component, vráť null (radšej chýbajúci údaj než nesprávny).
